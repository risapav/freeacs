...
function check_tomcat_installation {
  tomcatok=`grep tomcat8 /etc/passwd | wc -l`
  if [ $tomcatok != '1' ] ; then
    echo "The command 'grep tomcat8 /etc/passwd' doesn't seem to return the expected"
	echo "'tomcat8' string, indicating that this user is missing from the system. This"
	echo "in turns indicates that tomcat8 is not installed. Please correct this before"
	echo "you continue"
	exit
  else 
    echo "Tomcat 8 OK"
...
function prepare_tomcat {
  ...
  chown tomcat8:tomcat8 /etc/ssl/certs/java/cacerts
}
...
function tomcat_setup {
  mkdir /var/lib/tomcat8/shell 2> /dev/null
  echo ""
  ...
  do
	...
	do
      overwrite=''
      if [ -f /var/lib/tomcat8/common/$propertyfile ] ; then
        diff /var/lib/tomcat8/common/$propertyfile $propertyfile > tmp
        if [ -s tmp ] ; then
		  ...
		  mv -f $propertyfile /var/lib/tomcat8/common
        fi
	  else
	    mv -f $propertyfile /var/lib/tomcat8/common
      fi      
  ...
  for propertyfile in "${propertyfiles[@]}"
  do
    if [ -f /var/lib/tomcat8/shell/$propertyfile ] ; then
      diff /var/lib/tomcat8/shell/$propertyfile $propertyfile > tmp
      if [ -s tmp ] ; then
        echo "  $propertyfile diff found, added diff to config-diff.txt - please inspect!"
        echo "$propertyfile diff:" >> config-diff.txt
        echo "------------------------------------------------" >> config-diff.txt
	    ...
	    mv -f $propertyfile /var/lib/tomcat8/shell
      fi
	else
	  mv -f $propertyfile /var/lib/tomcat8/shell
    fi      
  ...
  mv *.war /var/lib/tomcat8/webapps
  mv *.jar /var/lib/tomcat8/shell
  ...
  ln -s /var/lib/tomcat8/webapps/web /var/lib/tomcat8/webapps/ROOT
  
  # Changes all ownership and permissions - tomcat8 user owns everything
  chown -R tomcat8:tomcat8 /var/lib/tomcat8
  chmod g+w /var/lib/tomcat8/common /var/lib/tomcat8/webapps /var/lib/tomcat8/shell
  chmod g+s /var/lib/tomcat8/common /var/lib/tomcat8/webapps /var/lib/tomcat8/shell
  echo "All file ownership and permissions have been transferred to the tomcat8 user"
  echo ""
...
function shell_setup {
  ...
  echo "cd /var/lib/tomcat8/shell" > /usr/bin/fusionshell
  echo "java -jar shell.jar" >> /usr/bin/fusionshell
  chmod 755 /usr/bin/fusionshell
 
  verified='n'
  until [ $verified == 'y' ] || [ $verified == 'Y' ]; do
    read -p "What is your Ubuntu username (will add tomcat8 group to this user): " ubuntuuser
    read -p "Is [$ubuntuuser] correct? (y/n) " verified
  done
  usermod -a -G tomcat8 $ubuntuuser
...
read -p "Upgrade Ubuntu, install mysql-5.7, openjdk-1.8 and tomcat-8? (y/n) " answer
echo ""
case $answer in
  [Yy]* ) install_basic ;;
esac
